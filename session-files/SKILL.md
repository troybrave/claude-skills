...
4. If **no match** → **Auto-detect from existing Sessions index file** (see below)
5. If auto-detect fails → ASK user: "What abbreviation should I use for this project's session index? (or type 'skip' to skip indexing)". If user says skip or you can't determine, set `{project_root}` = null
6. If working directory is **outside the vault** → `{project_root}` = null (skip indexing)

### Auto-Detect Sessions Index (Fallback)

When the Known Projects Table has no match, search for an existing `* Sessions.md` file by walking up the directory tree from the current working directory toward the vault root.

**Steps:**
1. Starting at the current working directory, glob for `* Sessions.md` in that directory
2. If not found, move one directory up and repeat — stop when you reach `{vault_root}`
3. If a file matching `{ABBREV} Sessions.md` is found:
   - `{project_root}` = the directory containing the file (with trailing `/`)
   - `{sessions_index_file}` = full path to the found file
   - Read the file's YAML frontmatter to extract `project` → `{project_name}` and `abbreviation` → `{project_abbrev}`
   - If frontmatter is missing or unreadable, parse from the filename: `{ABBREV}` = everything before ` Sessions.md`
4. If no `* Sessions.md` found anywhere up the tree → auto-detect fails, fall through to step 5 (ask user)

**Why this works:** Any project that already has a Sessions index file (created manually or by this skill) is automatically recognized on future sessions — no registration needed. The Known Projects Table is reserved for projects that need special path-keyword matching (e.g., when CWD doesn't contain a distinctive keyword).
