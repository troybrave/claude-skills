...
function parseYamlConfig(content) {
  // Simple YAML parser for config block
...
function checkInvariants(config) {
  ...
  // I1: if gateway_type == mcp, lifecycle_mode must be claude_managed or auto
  ...
  // I2: if lifecycle_mode == gateway_managed, gateway_type must be cli or http
  ...
  // I4: if fallback_type == cli, fallback_cli_path must be non-empty
  ...
  // I5: if fallback_type == http, fallback_http_url must be non-empty
...
function checkSecrets(content) {
  ...
  // Layer 2: Key name detection (simplified)
...
function checkDependencies(config) {
  ...
  if (config.gateway_type === 'cli' || config.lifecycle_mode === 'gateway_managed') {
    ...
    if (config.cli_path) {
      const cliPath = config.cli_path.replace(/^~/, process.env.HOME);
