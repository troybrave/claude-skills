...
**rclone fallback:** `sync-to-drive.cjs` and `send-meeting-email.cjs` automatically detect when the local Google Drive mount is unavailable and fall back to the `endlesswinning` rclone remote (configured in `~/.config/rclone/rclone.conf`). No manual intervention is needed.
